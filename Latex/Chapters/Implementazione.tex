...
La procedura usata in questa tesi segue la tecnica di costruzione FF-QRAM 
(Flip-flop QRAM) proposta da Park, Petruccione e Rhee \cite{petruccione}. 
...
L'idea soggiacente al modello della \ac{FF-QRAM} è rappresentata in Figura,
% inserisci figura e riferimento **********************************************
...
\end{equation}
Il secondo giro registra i prossimi dati $\vec{d}^{(l+1)}$ e $\theta^{(l+1)}$: 
\begin{equation}
...
% Hanno anche eseguito test su hardware reale? ***********************************
...
% qui non abbiamo più a che fare con valori binari, ma con valori reali
...
Dopo aver preparato lo stato, il circuito quantistico 
...
nuovo input. 
% la scelta di vettori normalizzati... *****************
